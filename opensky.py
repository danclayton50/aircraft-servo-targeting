...
class OpenSkyApi(object):
    ...
    def get_locallatlongalt(self,bound):
        '''
        Bound is 4 place tuple with minlat,maxlat,minlong,maxlong
        '''
        localac = self.get_states(bbox=bound).states
        values = []
        for ac in localac:
            cs,lat,lon,alt,ground = ac['callsign'],ac['latitude'],ac['longitude'],ac['geo_altitude'],ac['on_ground']
           
            if None not in (cs,lat,lon,alt,ground) :
                values.append([cs,lat,lon,alt,ground])
        return values